...
`MIN_FIT_SCORE`=45 · `MAYBE_MIN_SCORE`=30 · `PREFILTER_MIN`=25 · `MAX_LLM_CALLS`=120 (per track) · `MAX_AGE_DAYS`=21 · `MAX_PUSHES_PER_RUN`=15 (overflow rolls to next run) · `SCORE_BATCH`=8 · `SCORING_MODEL`=deepseek-chat (niche) · `EASY_SCORING_MODEL`=deepseek-chat (easy) · `PREFILTER_MODEL`=deepseek-chat · `DEEPSEEK_BASE_URL`=https://api.deepseek.com · `SEEN_FILE` · `MATCHES_FILE` · `DEADLINE_SOON_DAYS`=7 · `DEADLINE_WATCH_DAYS`=21 · `DEADLINE_MAX_CALLS`=12 · `DEADLINE_PAGE_CHARS`=6000 · `DIGEST_ENABLED`=1 · `DIGEST_HOUR_UTC`=7 · `DIGEST_HOUR_UTC_2`=15 · `DIGEST_TOP_N`=8 (`DIGEST_FORCE`=1 to force). *(Score/digest/age knobs are in temporary recall mode — see Changelog.)*
