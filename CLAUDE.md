...
**"No Easy Jobs for Rose"** — broad, well-paid jobs OUTSIDE her exact field that she
can land EASILY (clearly qualified/overqualified or with a stated hook; strictly Munich
or remote; its own rubric, sources, state files and Telegram chat; same bot).

> **👤 CANDIDATE — read her CV first.** Her actual CV is in the repo root as
> **`CV_Sevil_Zafarmandi.pdf`** (gitignored — personal doc; present locally for every
> chat to read). She is **Dr.-Ing. Sevil Zafarmandi, Architect & Building Scientist** at
> TUM (Chair of Building Technology & Climate-Responsive Design): personalized thermal
> comfort, building physics, climate-responsive design, **building & energy simulation**
> (EnergyPlus, ENVI-met, RayMan, DIALux, Ecotect), urban-microclimate field campaigns,
> and **AI/ML applied to the built environment** — **Python + ML only (NO SQL/R)**, not a
> generic data/economic/finance analyst. English C1, German B2, Turkish C1. The rubrics
> are tuned to THIS profile — read the CV before re-tuning fit.
...
- 2026-06-27 — **Her CV added to the repo + easy-track goal clarified + seen-state reset.** Her real CV lives at repo-root `CV_Sevil_Zafarmandi.pdf` (gitignored; see the 👤 callout near the top) — every chat should read it before tuning fit. `EASY_RUBRIC` goal reworded to the manager's spec: broad jobs OUTSIDE her niche that she lands EASILY (clearly/over-qualified or with a hook) AND that pay above the floor — NOT a perfect-fit requirement, but NOT roles she'd be screened out of. **`seen.json` + `seen_easy.json` reset to `{}`** so the whole pool re-scores under the corrected rubric and the relevant jobs re-surface; a fresh run was triggered right after. (Niche pushes still 403 until Sevil unblocks `@NoJobsforRoseBot`.)
